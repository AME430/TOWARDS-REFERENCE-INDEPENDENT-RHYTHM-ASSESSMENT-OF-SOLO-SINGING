...
def EstimatePitch(path,label,singer_name,song_name):  # 检测pitch

    original1 = path + label
    # ori_sig, rate = librosa.load(original1, sr=16000)
    ...
    if not os.path.exists(path+ '/nosilence'):
        os.mkdir(path+ '/nosilence')
    wav.write(
        path+ '/nosilence/'+ label,
        rate, np.int16(ori_sig * 32767))  # 写入新的去掉silence的wav

    original_pitch_file = 'C:/DataBaker_songs/ph_for_rhy/' + singer_name+'_'+ song_name +label.replace('.wav','.pitch')
    extract_pitch(
        path+ '/nosilence/'+ label,
        original_pitch_file, 0.01)


def runph(path,song_name,singer_name):
    global NFFT
    NFFT = 512
    mother_dir = path + os.sep + 'badsample_changerhythm' + os.sep + song_name + os.sep
    # label_list = ['_ori','_0','_1','_2','_3','_4','_speech_snippet']
    # label_list = ['_ori','_2','_speech_snippet']
    # label_list = ['_ori','_0','_speech_snippet']

    files = os.listdir(mother_dir)
    for file in files:
        if os.path.splitext(file)[1] == '.wav' and file.split('.')[1].split('_')[1]=='full':
            EstimatePitch(mother_dir,file,singer_name,song_name)

...
if __name__ == '__main__':
    ...
    configure('D:/nuspro/logg/MelSpec_ph_reg_lr0.0001_big_ELU_Adam_noteacc_bws-1to1', flush_secs=2)
    ...
    train_dataset = SpectralDataset('D:/nuspro/dilldata_ph/test/train_1.dill', 0, rep_params)
    # train_dataloader = SpectralDataLoader(train_dataset, batch_size=10, num_workers=4, shuffle=True)
    train_dataloader = SpectralDataLoader(train_dataset, batch_size=1, num_workers=1, shuffle=True)
    # pdb.set_trace()

    # test_dataset = SpectralDataset('/data07/huanglin/SingEval/LeaderboardData/data_ph/test_1.dill', 0, rep_params)
    test_dataset = SpectralDataset('D:/nuspro/dilldata_ph/test/test_1.dill', 0, rep_params)
    ...
    valid_dataset = SpectralDataset('D:/nuspro/dilldata_ph/test/val_1.dill', 0, rep_params)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (data) in enumerate(train_dataloader):
            inputs, targets, ph_notes = data  # 封装进dill中
        ...
        if val_pearson > best_val_corr:
            ...
            # torch.save(model, './models_hybrid/model_SpectralCRNN_reg_lr0.0001_big_ELU_Adam_noteacc_bws-1to1_mel_ph_corr')
            torch.save(model, 'D:/nuspro/model/model_SpectralCRNN_reg_lr0.0001_big_ELU_Adam_noteacc_bws-1to1_mel_ph_corr')
